...
func (_q *OrganizationQuery) QueryUser() *UserQuery {
...
func (_q *OrganizationQuery) QueryAsset() *AssetsQuery {
...
func (_q *OrganizationQuery) WithUser(opts ...func(*UserQuery)) *OrganizationQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *OrganizationQuery) WithAsset(opts ...func(*AssetsQuery)) *OrganizationQuery {
	query := (&AssetsClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(organization.FieldOrganizationName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Organization.Query().
//		Select(organization.FieldOrganizationName).
//		Scan(ctx, &v)
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *Organization, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAsset; query != nil {
		if err := _q.loadAsset(ctx, query, nodes,
			func(n *Organization) { n.Edges.Asset = []*Assets{} },
			func(n *Organization, e *Assets) { n.Edges.Asset = append(n.Edges.Asset, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedAsset {
		if err := _q.loadAsset(ctx, query, nodes,
			func(n *Organization) { n.appendNamedAsset(name) },
			func(n *Organization, e *Assets) { n.appendNamedAsset(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrganizationQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.UserColumn), fks...))
	}))
...
func (_q *OrganizationQuery) loadAsset(ctx context.Context, query *AssetsQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Assets)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(assets.FieldOrganizationID)
	}
	query.Where(predicate.Assets(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.AssetColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *OrganizationQuery) WithNamedAsset(name string, opts ...func(*AssetsQuery)) *OrganizationQuery {
	query := (&AssetsClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
