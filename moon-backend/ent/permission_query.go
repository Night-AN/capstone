...
//		GroupBy(permission.FieldPermissionName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Permission.Query().
//		Select(permission.FieldPermissionName).
//		Scan(ctx, &v)
...
func (_q *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	_spec.Node.Schema = _q.schemaConfig.Permission
	...
	if query := _q.withRole; query != nil {
		if err := _q.loadRole(ctx, query, nodes,
			func(n *Permission) { n.Edges.Role = []*Role{} },
			func(n *Permission, e *Role) { n.Edges.Role = append(n.Edges.Role, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedRole {
		if err := _q.loadRole(ctx, query, nodes,
			func(n *Permission) { n.appendNamedRole(name) },
			func(n *Permission, e *Role) { n.appendNamedRole(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *PermissionQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(_q.schemaConfig.Permission)
	ctx = internal.NewSchemaConfigContext(ctx, _q.schemaConfig)
...
func (_q *PermissionQuery) WithNamedRole(name string, opts ...func(*RoleQuery)) *PermissionQuery {
	query := (&RoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedRole == nil {
		_q.withNamedRole = make(map[string]*RoleQuery)
	}
	_q.withNamedRole[name] = query
	return _q
}
