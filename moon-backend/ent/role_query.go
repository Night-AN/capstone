...
type RoleQuery struct {
	...
	withNamedPermission map[string]*PermissionQuery
...
func (_q *RoleQuery) QueryPermission() *PermissionQuery {
...
func (_q *RoleQuery) WithPermission(opts ...func(*PermissionQuery)) *RoleQuery {
	query := (&PermissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withPermission; query != nil {
		if err := _q.loadPermission(ctx, query, nodes,
	...
	for name, query := range _q.withNamedUser {
		if err := _q.loadUser(ctx, query, nodes,
			func(n *Role) { n.appendNamedUser(name) },
			func(n *Role, e *User) { n.appendNamedUser(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedPermission {
		if err := _q.loadPermission(ctx, query, nodes,
			func(n *Role) { n.appendNamedPermission(name) },
			func(n *Role, e *Permission) { n.appendNamedPermission(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *RoleQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Role, init func(*Role), assign func(*Role, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		joinT.Schema(_q.schemaConfig.UserRole)
...
func (_q *RoleQuery) loadPermission(ctx context.Context, query *PermissionQuery, nodes []*Role, init func(*Role), assign func(*Role, *Permission)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Role)
	nids := make(map[uuid.UUID]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		joinT.Schema(_q.schemaConfig.RolePermission)
...
func (_q *RoleQuery) WithNamedUser(name string, opts ...func(*UserQuery)) *RoleQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithNamedPermission tells the query-builder to eager-load the nodes that are connected to the "permission"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithNamedPermission(name string, opts ...func(*PermissionQuery)) *RoleQuery {
	query := (&PermissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedPermission == nil {
		_q.withNamedPermission = make(map[string]*PermissionQuery)
	}
	_q.withNamedPermission[name] = query
	return _q
}
