...
func (rr *roleRepository) GetRolePermissions(ctx context.Context, roleID uuid.UUID) ([]aggregate.Permission, error) {
	...
	err := rr.db.WithContext(ctx).Table("systems.permission").Select("systems.permission.permission_id, systems.permission.permission_name, systems.permission.permission_description, systems.permission.permission_code, systems.permission.sensitive_flag, systems.permission.created_at, systems.permission.updated_at").Joins("JOIN systems.permission_role ON systems.permission.permission_id = systems.permission_role.permission_id").Where("systems.permission_role.role_id = ?", roleID).Find(&permissions).Error
